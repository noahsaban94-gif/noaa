...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // Active (non-delivered) orders vs Delivered
  const deliveredCount = baseOrders.filter((o) => o.status === 'נמסר באתר').length;
  const activeCount = baseOrders.filter((o) => o.status !== 'נמסר באתר').length;

  const filteredOrders = baseOrders.filter((o) => {
    const isDelivered = o.status === 'נמסר באתר';

    // Hide delivered orders from daily schedule unless explicitly toggled or filtered
    if (isDelivered && !showDelivered && statusFilter !== 'נמסר באתר') {
      return false;
    }

    const matchesStatus =
      statusFilter === 'all'
        ? (showDelivered ? true : !isDelivered)
      ...
      <div className="space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-800 font-bold">
              {filteredOrders.length} {showDelivered ? 'הזמנות במוצג' : 'פעילות בסידור'}
            </span>
            {deliveredCount > 0 && !showDelivered && (
              <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/80 font-bold inline-flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                <span>{deliveredCount} סופקו (מוסתרות)</span>
              </span>
            )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-100/60 p-2 rounded-2xl border border-slate-200/80">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 text-xs">
            ...
            {[
              { id: 'all', label: `פעילות (${activeCount})` },
            ...
            {/* Toggle button to show or hide delivered orders */}
            {deliveredCount > 0 && (
              <button
                type="button"
                onClick={() => {
                  if (statusFilter === 'נמסר באתר') {
                    setStatusFilter('all');
                ...
                className={`px-2.5 py-1 rounded-xl font-bold whitespace-nowrap transition text-xs cursor-pointer flex items-center gap-1.5 ${
                  showDelivered || statusFilter === 'נמסר באתר'
                    ? 'bg-emerald-600 text-white shadow-xs'
                    : 'bg-white text-emerald-700 border border-emerald-200 hover:bg-emerald-50'
                }`}
                title={showDelivered ? 'הסתר הזמנות שסופקו מסידור היום' : 'הצג הזמנות שסופקו היום'}
              >
                {showDelivered ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                <span>
                  {showDelivered
                    ? `הסתר סופקו (${deliveredCount})`
                    : `סופקו היום (${deliveredCount} מוסתרות)`}
                </span>
              </button>
            )}
          </div>
        ...
        {filteredOrders.length === 0 ? (
          <div className="p-8 win-card rounded-2xl text-center space-y-3 border border-slate-200/80">
            {activeCount === 0 && deliveredCount > 0 && !showDelivered ? (
              <>
                ...
                <div>
                  <p className="text-sm font-extrabold text-slate-900">כל ההזמנות של היום סופקו בהצלחה! 🏆</p>
                  <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
                    כל {deliveredCount} ההזמנות בסידור היום נמסרו באתר הלקוח והוסרו אוטומטית מסידור העבודה הפעיל.
                  </p>
                </div>
                <div className="pt-2 flex items-center justify-center gap-2">
                  <button
                    onClick={() => setShowDelivered(true)}
                    className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold inline-flex items-center gap-1.5 shadow-sm transition active:scale-95"
                  >
                    <Eye className="w-4 h-4" />
                    <span>הצג {deliveredCount} הזמנות שסופקו</span>
                  </button>
                  <button
                    onClick={onOpenNewOrder}
                    className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold inline-flex items-center gap-1.5 transition active:scale-95"
                  >
              ...
              <>
                ...
                <p className="text-xs text-slate-500 max-w-md mx-auto">
                  {deliveredCount > 0 && !showDelivered
                    ? `שים לב: ${deliveredCount} הזמנות בסטטוס "סופק" מוסתרות כרגע מסידור היום.`
                ...
                <div className="pt-1 flex items-center justify-center gap-2">
                  ...
                  {deliveredCount > 0 && !showDelivered && (
                    <button
                      onClick={() => setShowDelivered(true)}
                      className="px-3.5 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 text-xs font-bold transition"
                    >
                      הצג {deliveredCount} סופקו
                    </button>
                  )}
                  <button
                    onClick={onOpenNewOrder}
