...
// In-Memory live database initialized with the morning operational report orders (טאב: דוח_בוקר_מבצעי) + multi-day schedule
let ordersStore: ServerOrder[] = [
  ...
  },
  {
    ...
    wazeUrl: 'https://www.waze.com/ul?q=%D7%A1%D7%95%D7%A2%D7%95%D7%9C%D7%95%D7%91%2018%2C%20%D7%94%D7%A8%D7%A6%D7%9C%D7%99%D7%94&navigate=yes',
    status: 'נמסר באתר',
    date: '2026-09-10',
      ...
      if (sheetRes.ok) {
        ...
        if (data.values && Array.isArray(data.values)) {
          ...
          if (uniqueOrders.length > 0) {
            const sheetOrderNums = new Set(uniqueOrders.map((o) => o.orderNumber));
            const existingOtherOrders = ordersStore.filter((o) => !sheetOrderNums.has(o.orderNumber));
            ordersStore = [...uniqueOrders, ...existingOtherOrders];
          }
        }
      }
    }
